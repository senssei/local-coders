...
it touches. Private, git-ignored notes may exist in `scratch/` (for example `scratch/AFTERNOON.md`); they are context, not truth.
...
`/sdlc` (Claude Code, Antigravity) or "use the sdlc skill" (Codex, Cursor and others) and needs nothing else from you.
