...
(Claude Code: Agent tool with `general-purpose`; Codex: a new session or subagent; Antigravity: `invoke_subagent` with `TypeName: "self"` or a new session), give it only:
...
spec as the only truth. A local model (`local-coder` `review`) may be used as an *extra* reviewer, never as the independent one.
...
- Fix correctness, security and honest-routing findings through `sdlc-implement`: reproduce with a test, prove it red with
