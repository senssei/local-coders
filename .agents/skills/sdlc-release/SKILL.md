...
`lint`, `tests` and `changelog` must be `PASS` (`lint` may be `SKIP` when ruff is not installed; say so). On `FAIL`, fix the cause
and rerun. Never edit the script or skip a check to get green. Report the actual output.
...
- A version bump (`local_coder/__init__.py` `__version__`, and its copy under `packages/antigravity-local-coder/`) and moving
  `[Unreleased]` under a dated heading happen **only** when the operator is cutting a release.
...
hooks. If commits are GPG-signed and the agent waits for a passphrase, stop and ask the operator to commit; do not disable signing.

## 5. Publishing (only if asked, step by step)

Tags, GitHub releases and pushes each need their own explicit ask. Issues and comments on GitHub (including on `prism-local`) are
outward actions as well.
