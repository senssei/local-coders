...
`local-coder` calls Ollama's native `/api/chat` with `num_ctx` 8192 (override with `LOCAL_CODER_NUM_CTX`), so the prompt and up to `--max-tokens` (default 4096) of output fit in the context window. The `/v1` endpoint would ignore `num_ctx`.
